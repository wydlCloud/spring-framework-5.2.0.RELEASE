...
public abstract class AbstractBeanDefinitionReader implements BeanDefinitionReader, EnvironmentCapable {
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		// 这里取得ResourceLoader，使用getResourceLoader
		ResourceLoader resourceLoader = getResourceLoader();
		if (resourceLoader == null) {
			throw new BeanDefinitionStoreException(
					"Cannot load bean definitions from location [" + location + "]: no ResourceLoader available");
		}
		// 这里对Resource的路径模式进行解析，比如我们设定的各种ant格式的路径定义，
		// 得到需要的Resource集合，这些Resource集合指向我们已经定义好的BeanDefinition信息，可以是多个文件
		if (resourceLoader instanceof ResourcePatternResolver) {
			// Resource pattern matching available.
			try {
				// 调用DefaultResourceLoader的getResources完成具体的Resource定位工作
			...
			// 调用DefaultResourceLoader完成具体的Resource资源定位工作
